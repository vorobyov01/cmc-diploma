...
\usepackage{listings}
\lstset{basicstyle=\ttfamily\small, frame=none}
\usepackage{parskip}

\title{\textbf{Scaling Neural Network Verification with\\
Tensor Parallelism and Fully Sharded Data Parallelism}}

\author{%
  Sergei Vorobyov \\
  Lomonosov Moscow State University \\
  \texttt{sergei.vorobyov01@gmail.com}
  \and
  Evgeny Ilyushin \\
  Lomonosov Moscow State University
}
...
verification framework. \textbf{Tensor Parallelism (TP)} shards both weight and
...
substitution inside sharded zones. \textbf{Fully Sharded Data Parallelism
...
baseline memory drops by 80--90\%, peak memory by 34--39\% on wide MLPs.
...
the entire set of admissible inputs \citep{Guidotti2019Enhancing}. Formal
...
(SAT), a counterexample is returned \citep{marzari2025advancingneuralnetworkverification}.
...
$\|\mathbf{x}-\mathbf{x}_0\|_p \leq \varepsilon$ \citep{DBLP:journals/corr/abs-2002-03339}.
Beyond image classifiers, functional specifications over physical variables are
used in control tasks such as ACAS~Xu \citep{Demarchi2023SupportingStandardization}.

\subsection{The VNN-LIB Standard and VNN-COMP}

Prior to 2020, each verifier used its own format for network and property
descriptions. The VNN-LIB standard \citep{VNNLIBWebsite} unified the
...
\citep{arXiv2212VNNCOMP2022, arXiv2312VNNCOMP2023, arXiv2412VNNCOMP2024},
...
reduces polynomially to it \citep{arXiv2403RobustnessVerification}. For
smooth activations (Sigmoid, Tanh, GELU) the complexity is
$\exists\mathbb{R}$-complete \citep{NeurIPS2021ExistsRComplete}. For certain
classes of recurrent networks, reachability is undecidable
\citep{IJCAI2018ReachabilityDNN}. In practice this means a \emph{complete}
...
$\alpha,\beta$-CROWN \citep{AlphaBetaCrownVNNCOMP23}---winner of VNN-COMP
...
\begin{enumerate}
\item We implement \textbf{Tensor Parallelism} for \texttt{auto\_LiRPA} via
  ...
  zones.
\item We implement \textbf{FSDP} for bound propagation in ${\approx}100$ lines,
  producing bitwise-identical bounds with 34--39\% peak and 80--90\% baseline
  memory savings on wide MLPs.
\item We integrate FSDP with \textbf{complete verification} ($\beta$-CROWN+BaB),
  introduce a \texttt{force\_synchronous} flag for fair comparison, and extend
  sharding to \textbf{convolutional layers} (\texttt{BoundConv}).
\item We identify \textbf{per-neuron alpha tensors}---not weights---as the true
...
\textbf{IBP (Interval Bound Propagation)} \citep{CROWNPaper} propagates
...
\textbf{CROWN} \citep{CROWNPaper} computes tight linear bounds via a backward
...
\textbf{$\alpha$-CROWN} \citep{AlphaBetaCrownVNNCOMP23} promotes the fixed
...
\emph{Complete verification} \citep{betaCROWN2021} guarantees a definitive
answer via \textbf{Branch-and-Bound (BaB)}: when bounds are insufficient, the
...
$\alpha,\beta$-CROWN \citep{AlphaBetaCrownVNNCOMP23} unifies all methods above.
...
\citep{betaCROWN2021} is encoding ReLU-split constraints directly into the
...
\citep{NeurIPS2024BnbCuttingPlanes} generates constraints from logical
...
low-dimensional control tasks such as ACAS~Xu \citep{IntervalReachabilityAnalysis}.

\textbf{nnenum} \citep{NNEnumPaper} (winner of VNN-COMP 2020) dynamically
...
\textbf{Marabou 2.0} \citep{arXiv2401Marabou2} transforms the combinatorial
...
\textbf{NeuralSAT} \citep{NeuralSATRepo} adapts CDCL (Conflict-Driven Clause
...
\citep{AutoLiRPARepo}, the library underlying $\alpha,\beta$-CROWN.
...
We briefly define the two strategies we adapt; readers familiar with
distributed training may skip to Section~\ref{sec:why}.

\textbf{Tensor Parallelism (TP)} \citep{MegatronLM2020} splits matrix
...
\textbf{Fully Sharded Data Parallelism (FSDP)} \citep{ZeROPaper} shards each
...
\textbf{Pipeline Parallelism (PP)} \citep{GPipePaper} assigns consecutive
...
\citep{moore2009interval}---bound widths grow as $O(\rho^k)$ with zone depth
...
\begin{equation}
  M_\text{peak}^\text{FSDP} \approx h^2\!\left(\frac{d}{P} + 1 + C\right).
  \label{eq:fsdp-memory}
\end{equation}
The ratio to single-GPU is $(d/P + 1 + C)/(d + C)$.
At $d{=}4$, $C{\approx}4$, $P{=}2$: $7/8 = 0.875$, i.e.\ ${\sim}12.5\%$
reduction---matching the observed 34\% (baseline overhead from NCCL buffers
and metadata are not counted here). Baseline weight storage:
$M_W^\text{FSDP} = d h^2/P + h^2$, ratio $(1/P + 1/d)$; at $d{=}8$, $P{=}2$
this gives $0.625$, matching the observed 80--90\%.
...
Megatron-LM pattern \citep{MegatronLM2020}.
...
models \citep{arXiv2212VNNCOMP2022}; $\varepsilon{=}0.02$, $L_\infty$.
...
\begin{table}[h]
\caption{FSDP=2 vs.\ single-GPU memory.
``---'' indicates savings are negligible ($h \leq 1024$: AllGather overhead
$>$ weight savings).}
...
\begin{tabular}{lrrrr}
\toprule
Model & \multicolumn{2}{c}{Baseline (MB)} & \multicolumn{2}{c}{Peak (MB)} \\
...
Baseline savings: 57--90\%, growing with model size. At $h \geq 4096$ savings
stabilise at ${\sim}82\%$, matching the theoretical limit $1/P + 1/d$
(Section~\ref{sec:theory}). Peak savings are 34--39\%: CROWN $A$-matrices are
\emph{not} sharded and are comparable in size to weights at $d{=}4$;
the weight fraction grows at $d{=}8$, improving savings to 39\%.
...
\paragraph{Protocol.}
\texttt{mnist-net\_256x6} \citep{arXiv2212VNNCOMP2022}, $\varepsilon{=}0.05$;
...
parameters, 0.3~MB weights) \citep{arXiv2312VNNCOMP2023}. Transformers use
...
Validated on CIFAR-100 ResNets \citep{arXiv2412VNNCOMP2024}.
...
ResNet-large (${\sim}95$~MB weights, $>4$~GB alpha tensors): AllGather overhead
(${\sim}30$~MB) exceeds weight-sharding savings at $P{=}2$, confirming that
alpha tensors dominate. The \textbf{unsat} result---property proved by
$\alpha$-CROWN at initialisation, without BaB---is the first complete
verification result obtained under FSDP for a convolutional VNN-COMP model.

\paragraph{Summary.}
FSDP delivers meaningful peak-memory savings (34--39\%) only when weights
dominate, i.e.\ in incomplete (CROWN-only) verification of wide MLPs. In
$\alpha$-CROWN+BaB the bottleneck is alpha tensors; sharding them is the
critical next step.
...
\textbf{Bound propagation.}
CROWN \citep{CROWNPaper} introduced linear relaxation via a backward pass.
$\alpha$-CROWN \citep{AlphaBetaCrownVNNCOMP23} and $\beta$-CROWN
\citep{betaCROWN2021} extended it with optimisable slopes and dual-variable
encoding of branching constraints. BICCOS \citep{NeurIPS2024BnbCuttingPlanes}
adds cutting planes from the BaB tree. DeepPoly \citep{DeepPolyAbstractDomain}
uses polyhedral abstract interpretation; GPU-accelerated scaling was studied
in \citep{ScalingPolyhedralGPU}. OSIP \citep{arXiv2212OSIP} proposes optimised
symbolic interval propagation for tighter intermediate bounds.

\textbf{Complete verifiers.} Marabou 2.0 \citep{arXiv2401Marabou2} uses the
DeepSoI continuous objective. NeuralSAT \citep{NeuralSATRepo} employs CDCL.
nnenum \citep{NNEnumPaper} and VeriNet use SIP with dynamic precision switching.
MIPVerify \citep{arXiv1711MIPRobustness} encodes ReLU networks as MIPs.

\textbf{Parameter sharding for training.}
Megatron-LM \citep{MegatronLM2020} introduced intra-layer tensor parallelism
for Transformer training. ZeRO \citep{ZeROPaper} shards parameters, gradients,
and optimizer states. GPipe \citep{GPipePaper} introduced pipeline parallelism
with micro-batches.

\textbf{Multi-GPU verification.}
...
sharded zones. FSDP achieves smaller but still meaningful peak savings
(34--39\% in incomplete verification) with bitwise-exact bounds and seamless
integration with complete verification ($\beta$-CROWN+BaB).
