...
def run_experiment(model, x, eps, dev, rank, ws, label, method="CROWN"):
    ...
    mem_all = [torch.zeros(1, device=dev) for _ in range(ws)]
    dist.all_gather(mem_all, torch.tensor([mem_fsdp], device=dev))
    ...
    if rank == 0:
        ...
        max_fsdp_mem = max(m.item() for m in mem_all)
        savings = (1.0 - max_fsdp_mem / mem_single) * 100 if mem_single > 0 else 0
        ...
        print(f"  Memory: single={mem_single:.1f} MB, "
              f"FSDP(max)={max_fsdp_mem:.1f} MB, "
              f"savings={savings:.1f}%")
        return {
            "label": label, "method": method, "status": status,
            "lb_diff": lb_diff, "ub_diff": ub_diff,
            "mem_single": mem_single, "mem_fsdp": max_fsdp_mem,
            "savings_pct": savings
        }
    return None
...
def main():
    ...
    for hidden in [256, 1024, 4096, 8192, 16384]:
    ...
    if rank == 0 and results:
        ...
        print("SUMMARY")
        print("=" * 60)
        print(f"{'Model':<25} {'Status':<6} {'Single MB':>10} "
              f"{'FSDP MB':>10} {'Savings':>8} {'lb_diff':>10}")
        print("-" * 75)
        for r in results:
            print(f"{r['label']:<25} {r['status']:<6} "
                  f"{r['mem_single']:>10.1f} {r['mem_fsdp']:>10.1f} "
                  f"{r['savings_pct']:>7.1f}% {r['lb_diff']:>10.2e}")
